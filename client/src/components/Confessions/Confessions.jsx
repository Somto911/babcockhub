...
export default function Confessions() {
  ...
  return (
    <div className="pg on" id="pg-conf">
      <div className="pg-hd"><div className="pg-title">Anonymous</div></div>
      <div className="conf-composer">
        <div className="conf-note">Your identity is hidden — post anonymously</div>
        <textarea className="comp-ta" placeholder="Share what's on your mind…" rows={3} style={{ borderColor: 'rgba(139,92,246,.2)' }} value={text} onChange={(e) => setText(e.target.value)} />
        <div style={{ marginTop: 8, display: 'flex' }}>
          <button className="comp-sub" style={{ background: 'var(--purple)', boxShadow: '0 4px 18px var(--purple-dim)' }} onClick={submit}>Post</button>
        </div>
      </div>
      <div id="conf-list">
        {confs.map((c) => (
          <div className="conf" key={c.id}>
            <div className="conf-hd">
              <div className="conf-av">A</div>
              <div>
                <div className="conf-nm">Anonymous Student</div>
                <div className="conf-t">{c.t}</div>
              </div>
              <div className="more-btn" style={{ marginLeft: 'auto' }}>Flag</div>
            </div>
